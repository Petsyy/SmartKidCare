import { CalendarDays, UserRound, X, Utensils } from "lucide-react";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
type ViewModalProps = { viewingRow: FeedingRow | null; onClose: () => void };
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        ...
        onClick={onClose}
      >
        <DialogTitle className="sr-only">View Feeding Record</DialogTitle>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50/60 p-6 dark:bg-slate-950/30">
            ...
            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="rounded-xl border border-gray-200 bg-gray-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/70">
                ...
                <p className={valueClass}>{viewingRow.childName || "-"}</p>
              </div>
              <div className="rounded-xl border border-gray-200 bg-gray-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/70">
                <div className="mb-2 flex items-center gap-2 text-teal-600 dark:text-teal-400">
                ...
                <p className={valueClass}>{formatDate(viewingRow.date)}</p>
              </div>
              <div className="rounded-xl border border-gray-200 bg-gray-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/70">
                <p className={labelClass}>Status</p>
                <span
                  className={`inline-flex rounded-lg px-3 py-1.5 text-xs font-semibold ${viewingRow.status === "completed" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200" : "bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-200"}`}
                >
