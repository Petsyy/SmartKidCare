...
export function EditCenterModal({ center, onClose, onUpdate }: EditCenterModalProps) {
  const { register, handleSubmit, reset, formState: { errors }, } = useForm<EditCenterFormData>({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          {mutation.error && (
            <div className="mb-6 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-600 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-400">
              {mutation.error instanceof Error ? mutation.error.message : "Failed to update center"}
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              />
              <label htmlFor="editIsActive" className="text-sm font-medium text-gray-700 dark:text-slate-300">
                Center is Active
              </label>
            </div>
          </div>
