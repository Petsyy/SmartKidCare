import { z } from "zod";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { useMutation } from "@tanstack/react-query";
import { X, Building2 } from "lucide-react";
import type { DaycareCenter } from "@/api/daycare-center.api";

const addCenterSchema = z.object({
  name: z.string().min(1, "Center name is required"),
  barangay: z.string().min(1, "Barangay is required"),
  code: z.string().min(1, "Center code is required"),
  address: z.string().optional(),
  isActive: z.boolean(),
});

type AddCenterFormData = z.infer<typeof addCenterSchema>;

type AddCenterModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (data: Partial<DaycareCenter>) => Promise<void>;
};

export function AddCenterModal({
  isOpen,
  onClose,
  onAdd,
}: AddCenterModalProps) {
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<AddCenterFormData>({
    resolver: zodResolver(addCenterSchema),
    defaultValues: {
      name: "",
      barangay: "",
      code: "",
      address: "",
      isActive: true,
    },
  });

  const mutation = useMutation({
    mutationFn: async (data: AddCenterFormData) => {
      await onAdd(data);
    },
    onSuccess: () => {
      reset();
      onClose();
    },
  });

  if (!isOpen) return null;

  const onSubmit = (data: AddCenterFormData) => {
    mutation.mutate(data);
  };

  const handleClose = () => {
    reset();
    mutation.reset();
    onClose();
  };

  return (
    <div
      className="fixed inset-0 z-60 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm transition-all"
      onClick={handleClose}
    >
      <div
        className="w-full max-w-lg overflow-hidden rounded-3xl border border-white/20 bg-white/95 shadow-2xl backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/95"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-gray-200/50 bg-gradient-to-r from-teal-50 to-emerald-50/50 px-6 py-5 backdrop-blur-md dark:border-slate-700/50 dark:from-teal-900/30 dark:to-emerald-900/20">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-100 text-teal-600 dark:bg-teal-900/40 dark:text-teal-400">
              <Building2 size={20} />
            </div>
            <h3 className="text-lg font-bold tracking-tight text-gray-900 dark:text-slate-100">
              Add New Center
            </h3>
          </div>
          <button
            onClick={handleClose}
            className="cursor-pointer rounded-xl bg-white/50 p-2 text-gray-500 shadow-sm backdrop-blur-sm transition-all duration-200 hover:bg-white hover:text-gray-700 dark:bg-slate-800/50 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200"
            aria-label="Close modal"
          >
            <X size={18} />
          </button>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          {mutation.error && (
            <div className="mb-6 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-600 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-400">
              {mutation.error instanceof Error ? mutation.error.message : "Failed to add center"}
            </div>
          )}

          <div className="space-y-4">
            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                Center Name
              </label>
              <input
                type="text"
                {...register("name")}
                placeholder="e.g. Bonuan Boquig Child Development Center"
                className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none transition-colors focus:border-teal-500 focus:bg-white focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-100 dark:focus:border-teal-500"
              />
              {errors.name && (
                <p className="mt-1.5 text-xs text-rose-500">{errors.name.message}</p>
              )}
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                  Barangay
                </label>
                <input
                  type="text"
                  {...register("barangay")}
                  placeholder="e.g. Bonuan Boquig"
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none transition-colors focus:border-teal-500 focus:bg-white focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-100 dark:focus:border-teal-500"
                />
                {errors.barangay && (
                  <p className="mt-1.5 text-xs text-rose-500">{errors.barangay.message}</p>
                )}
              </div>
              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                  Center Code
                </label>
                <input
                  type="text"
                  {...register("code")}
                  placeholder="e.g. BB-01"
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none transition-colors focus:border-teal-500 focus:bg-white focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-100 dark:focus:border-teal-500"
                />
                {errors.code && (
                  <p className="mt-1.5 text-xs text-rose-500">{errors.code.message}</p>
                )}
              </div>
            </div>

            <div>
              <label className="mb-1 block text-sm font-semibold text-gray-700 dark:text-slate-300">
                Address (Optional)
              </label>
              <textarea
                {...register("address")}
                placeholder="Full address of the center"
                rows={2}
                className="w-full resize-none rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none transition-colors focus:border-teal-500 focus:bg-white focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-100 dark:focus:border-teal-500"
              />
            </div>

            <div className="flex items-center gap-2 pt-2">
              <input
                type="checkbox"
                id="isActive"
                {...register("isActive")}
                className="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500 dark:border-slate-600 dark:bg-slate-800"
              />
              <label
                htmlFor="isActive"
                className="text-sm font-medium text-gray-700 dark:text-slate-300"
              >
                Center is Active
              </label>
            </div>
          </div>

          <div className="mt-8 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={handleClose}
              disabled={mutation.isPending}
              className="rounded-xl border border-gray-200 px-5 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={mutation.isPending}
              className="inline-flex items-center justify-center rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500/30 disabled:cursor-not-allowed disabled:bg-teal-400"
            >
              {mutation.isPending ? "Adding..." : "Add Center"}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}
