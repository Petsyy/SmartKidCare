...
function VerificationStatus({
  ...
  return (
    <p
      aria-live="polite"
      className={`inline-flex items-center justify-center gap-2 rounded-lg border px-3 py-1.5 text-xs font-semibold shadow-sm ${classes}`}
    >
      {loading ? (
        <Loader2
          size={14}
...
function InfoCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-200/70 bg-white p-4 shadow-sm dark:border-slate-700/60 dark:bg-slate-800/80">
      ...
      {subvalue ? (
        <div className="mt-1.5 text-xs font-medium text-gray-500 dark:text-slate-400">
          {subvalue}
        </div>
      ) : null}
    </div>
  ...
  return (
    <div className="rounded-2xl border border-gray-200/70 bg-white p-4 shadow-sm dark:border-slate-700/60 dark:bg-slate-800/80">
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${isAvailable ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300" : "bg-slate-100 text-slate-600 dark:bg-slate-700/70 dark:text-slate-300"}`}
        >
...
function ContactCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-200/70 bg-white p-4 shadow-sm dark:border-slate-700/60 dark:bg-slate-800/80">
      <p className="mb-3 text-xs font-semibold uppercase tracking-[0.08em] text-slate-600 dark:text-slate-400">
        {label}
      </p>
      {name === emptyLabel ? (
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2">
            ...
            />
            <p className="text-[15px] font-medium leading-6 text-slate-900 dark:text-slate-100">
              {name}
            </p>
          </div>
          {email ? (
            <a
              href={`mailto:${email}`}
              className="flex items-start gap-2 text-[15px] font-medium text-teal-700 underline-offset-2 hover:underline dark:text-teal-300"
            >
          ...
          {phone ? (
            <div className="flex items-start gap-2 text-[15px] font-medium text-gray-500 dark:text-slate-400">
              <Phone
                size={14}
                className="mt-1 shrink-0 text-gray-400 dark:text-slate-500"
              />
              <span>{phone}</span>
            </div>
          ) : null}
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <InfoCard
          ...
          value={child.parentRelationship || "Not set"}
        />
      </div>

      <InfoCard
        label="Complete Home Address"
        value={child.homeAddress || "Not set"}
      />

      {child.daycareCenter ? (
        <InfoCard label="Daycare Center" value={child.daycareCenter.name} />
      ) : null}
    </div>
  );
}
    ...
    >
      <div className="rounded-3xl border border-gray-200/70 bg-white/80 p-5 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
        ...
        <p className="mt-1 text-sm font-medium text-gray-500 dark:text-slate-400">
          Secure links are signed for a short time window before opening.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <DocumentCard
            ...
            verification={
              blockchainProof
                ? blockchainProof.documents.birthCertificate.blockchainVerified
                : undefined
            }
          />
          <DocumentCard
            ...
            verification={
              blockchainProof
                ? blockchainProof.documents.parentId.blockchainVerified
                : undefined
            }
          />
        </div>
      ...
      {blockchainErrorMessage ? (
        <ErrorAlert
          ...
          message={blockchainErrorMessage}
        />
      ) : null}
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <InfoCard
                    label="Network"
                    value={blockchainProof.network || "N/A"}
                  />
                  <InfoCard
                    label="Transaction"
                    value={
                      <span className="font-mono text-xs">
                        {blockchainProof.txHash
                          ? formatTxDisplay(blockchainProof.txHash)
                          : "N/A"}
                      </span>
                    }
                    subvalue={
                      blockchainProof.anchoredAt
                        ? `Anchored ${formatDate(blockchainProof.anchoredAt)}`
                        : undefined
                    }
                  />
                </div>
