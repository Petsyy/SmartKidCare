...
import type { AuthUser } from "../types/enrollment-parent-credentials.types";
...
export const resetEnrollmentRequestParentPassword = async (
  ...
  const parent = await parentService.findParentByEmail(parentEmail);
  if (!parent) {
    throw new NotFoundError("Parent account");
  }

  const credentials = await parentService.resetParentPassword(parent);
...
export const getEnrollmentRequestParentCredentials = async (
  ...
  const credentials = parentService.getParentCredentials(parent);
