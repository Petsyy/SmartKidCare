...
import { AIServiceError } from "./core/gemini.service";
...
const buildScopedFollowUpQuestion = (
  role: string,
...
const parseRequest = (body: unknown, auth: AuthContext) => {
  ...
  if (role !== "parent")
    return { ok: false, status: 403, message: "AI chat is for parents only." };
...
export const handleAiChatRequest = async (
  ...
  try {
    ...
    return {
      ...
      body: {
        reply:
          "I couldn't process that. Please ask about attendance or feeding.",
      },
    };
  } catch (error) {
