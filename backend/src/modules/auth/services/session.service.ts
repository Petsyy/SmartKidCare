...
export interface SessionServiceDependencies {
  authUserRepository: typeof defaultAuthUserRepository;
  authChildRepository: typeof defaultAuthChildRepository;
}

export class SessionService {
  private authUserRepository: typeof defaultAuthUserRepository;
  private authChildRepository: typeof defaultAuthChildRepository;

  constructor(deps: Partial<SessionServiceDependencies> = {}) {
    this.authUserRepository = deps.authUserRepository || defaultAuthUserRepository;
    this.authChildRepository = deps.authChildRepository || defaultAuthChildRepository;
  }
  ...
  public async findUserByIdentifier(
    ...
    return this.authUserRepository.findByIdentifier(identifier, isAdminRoute);
  }
   ...
   */
  public sanitizeUserResponse(user: any): any {
   ...
   */
  public handleTeacherParentLogin(user: any) {
    const token = signAuthToken(String(user._id), user.role);
    return {
      token,
   ...
   */
  public async login(credentials: LoginCredentials, res: Response) {
    ...
    const user = await this.findUserByIdentifier(identifier, isAdminRoute);
    ...
    return this.handleTeacherParentLogin(user);
  }
  ...
  public async getUserById(userId: string): Promise<any> {
    const user = await this.authUserRepository.findByIdWithPopulate(userId);
  ...
  public async updateUserProfile(
    ...
    if (payload.username !== undefined) {
      ...
      const existingByUsername = await this.authUserRepository.findByUsernameExcluding(
        normalizedUsername,
        userId,
      );
    ...
    if (payload.email !== undefined) {
      ...
      const existingByEmail = await this.authUserRepository.findByEmailExcluding(
        normalizedEmail,
        userId,
      );
   ...
   */
  public async attachLinkedChildren(
    parentUsers: any[]
    ...
    const linkedChildren = await this.authChildRepository.findByParentIds(parentIds);
