        ...
        >
          ...
          <View className="flex-row flex-wrap">
            <View className="w-[48%] mr-[4%] mb-4">
              <View className="bg-green-50 rounded-2xl p-4 items-start border-l-4 border-green-500" style={{ shadowColor: "#059669", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 1 }}>
            ...
            <View className="w-[48%]">
              <View className="bg-red-50 rounded-2xl p-4 items-start border-l-4 border-red-500" style={{ shadowColor: "#DC2626", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 1 }}>
          ...
          <View className="mt-6 pt-4 border-t border-gray-100">
            ...
            <View className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <View className="h-full bg-green-500 rounded-full" style={{ width: `${attendanceRate}%` }} />
            </View>
          </View>
        </View>
      )}
      renderModalContent={(dayDetails, currentSelectedDay, selectedDateLabel, closeModal) => {
        ...
        return (
          <View className="flex-1 bg-black/60 items-center justify-center px-6">
            <View className="w-full rounded-3xl bg-white p-6" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 10 }}>
              <View className="mb-5">
                <View className="flex-row items-center">
                  ...
                  <View className="flex-1">
                    <Text className="text-xl font-bold text-gray-900">{selectedDateLabel}</Text>
                    <Text className="text-sm text-gray-500 mt-0.5">Attendance Details</Text>
                  </View>
                </View>
              </View>

              <View className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-5 border border-gray-100">
                <View className="bg-white rounded-xl p-4 mb-3" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}>
                  <Text className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">Status</Text>
                  <View className={`px-4 py-2 rounded-full self-start ${dayDetails?.status === "Present" ? "bg-green-100" : dayDetails?.status === "Absent" ? "bg-red-100" : "bg-gray-100"}`}>
                    <Text className={`text-base font-bold ${dayDetails?.status === "Present" ? "text-green-700" : dayDetails?.status === "Absent" ? "text-red-700" : "text-gray-700"}`}>
                      {currentSelectedDay ? getStatusForDay(currentSelectedDay) || "Not recorded" : "Not recorded"}
                    </Text>
                  </View>

                  <Text className="text-xs font-semibold text-gray-500 mb-2 mt-4 uppercase tracking-wider">Teacher</Text>
                  <Text className="text-base font-bold text-gray-800">{dayDetails?.teacherName || "Not available"}</Text>
                </View>

                <View className="bg-white rounded-xl p-4" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}>
                  <Text className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">Recorded At</Text>
                  <Text className="text-base font-bold text-gray-800">{recordedLabel}</Text>
                </View>
              </View>

              <Pressable onPress={closeModal} className="mt-5 bg-teal-600 rounded-xl py-4 active:opacity-80" style={{ shadowColor: "#14B8A6", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 3 }}>
