...
import {requestForgotPasswordOtp,verifyForgotPasswordOtp} from "@/src/api/authentication.api";
...
export default function ForgotPasswordOtpScreen() {
  ...
  const handleVerifyOtp = async () => {
    ...
    if (!email) {
      Alert.alert(
        "Invalid Request",
        "Missing email. Start forgot password again.",
      );
      return;
    }
  ...
  return (
    <AuthLayout onBack={() => router.replace("/(auth)/forgot-password")}>
      <View className="items-center mb-6">
        ...
        <Text className="text-xl text-teal-700 font-semibold text-center mt-1">
          {email || "your email"}
        </Text>
      </View>
      ...
      <TextInput
        ...
        className="w-full px-4 py-3 border border-gray-300 rounded-xl text-lg text-gray-900 text-center tracking-[6px]"
      />
